...
from mpc_vanilla.arm_model import PCCSoftArm
...
pcc_arm = PCCSoftArm(
    L_segs=robot_params['L_segs'],
    m=robot_params['m'],
    g_vec=robot_params['g'],
    d_eq=robot_params['d_eq'],
    K=robot_params['K']
)
# Load the pre-compiled CasADi functions
dynamics_func, shape_func = pcc_arm.dynamics_func, pcc_arm.shape_func
...
ode = {'x': x, 'p': p, 'ode': dynamics_func(x, p[0:6], p[6], p[7:10], p[10:13], ca.reshape(p[13:], 6, 6))}
...
for x in x_history:
    q = x[:6]
    segment1, segment2, segment3 = shape_func(q)
