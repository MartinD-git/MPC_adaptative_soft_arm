...
def main():
    ...
    '''first_point = np.array([0.3*pcc_arm.L_segs[0],0.0,1.8*pcc_arm.L_segs[0]])
    second_point = np.array([1.2*pcc_arm.L_segs[0],0.0,1.5*pcc_arm.L_segs[0]])
    xyz_circular_traj = np.vstack((np.tile(first_point,(num_iter//2+1,1)), np.linspace(first_point,second_point,num_iter//4),np.tile(second_point,(num_iter//4+N+1,1))))
    ...
    with tqdm(total=num_iter*SIM_PARAMETERS['dt'], desc="MPC loop", bar_format='{l_bar}{bar}| {n:.2f}/{total_fmt} [{elapsed}<{remaining}, {postfix}]') as pbar:
        for t in range(num_iter):
            try:
                ...
                u0 = mpc_step_acados(ocp_solver, pcc_arm.current_state, q_goal_value, N, MPC_PARAMETERS['u_bound'])#+ np.ones_like(u0)*0.1
