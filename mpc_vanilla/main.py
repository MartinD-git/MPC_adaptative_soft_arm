...
def main():
    ...
    d_par   = opti.parameter(pcc_arm.num_segments)
    K_par   = opti.parameter(2*pcc_arm.num_segments, 2*pcc_arm.num_segments)
    ...
    for i in range(N):
        p_i  = ca.vertcat(u[:, i], m_par, d_par, ca.reshape(K_par, (2*pcc_arm.num_segments)**2, 1))
