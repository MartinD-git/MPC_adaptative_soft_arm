...
def pcc_dynamics(arm,q, q_dot, tips, jacobians,water=False):
    ...
    if water:
        K_disturbance = np.diag([0.0,0.02,0.0,0.02])
    else:
        K_disturbance = np.diag([0.0,0.0,0.0,0.0])

    K = arm.K + ca.diag(ca.vertcat(0, p_adaptative[0], 0, p_adaptative[1]))+ K_disturbance
