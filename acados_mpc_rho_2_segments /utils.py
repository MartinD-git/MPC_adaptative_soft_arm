...
def pcc_dynamics(arm,q, q_dot, tips, jacobians):
    ...
    M_func = ca.Function('M_func', [q, rho_fluid], [M])
    G_func = ca.Function('G_func', [q, rho_fluid], [G])
    D_func = ca.Function('D_func', [q, q_dot, rho_fluid], [D])

    # Coriolis C
    M_q = M_func(q, rho_fluid)
    ...
    C_vec_func = ca.Function('C_vec_func', [q, q_dot, rho_fluid], [c_vec])
    ...
    K_term= K @ q_from_x'''
    M_term= M_func(q0[:2*num_segments], rho_fluid)+1e-6* ca.DM.eye(2*num_segments)
    C_term= C_vec_func(q0[:2*num_segments], q0[2*num_segments:], rho_fluid)
    G_term= G_func(q0[:2*num_segments], rho_fluid)
    D_term= D_func(q0[:2*num_segments], q0[2*num_segments:], rho_fluid) @ q_dot_from_x
    K_term= K @ q_from_x
