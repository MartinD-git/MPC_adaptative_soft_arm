...
def history_plot(pcc_arm,u_bound,xyz_traj=None, save=False, opti_index=None, sim_parameters=None):
    ...
    try:
        ...
        nopti_q_error = np.load(out_dir + "q_error.npy")
        nopti_xyz_error = np.load(out_dir + "xyz_error.npy")

        axs[0].plot(time_axis, q_error, label='RMSE jointspace', color='C0')
        axs[0].plot(time_axis, nopti_q_error, label='No adapt. RMSE jointspace', linestyle='--', color='C0')
        mean_error = np.convolve(q_error, np.ones(N_mean)/N_mean, mode='valid')
        mean_error_no_adapt = np.convolve(nopti_q_error, np.ones(N_mean)/N_mean, mode='valid')
        axs[0].plot(time_axis[-len(mean_error):], mean_error, label='Loop average', color='C1')
        axs[0].plot(time_axis[-len(mean_error_no_adapt):], mean_error_no_adapt, label='No adapt. Loop average', linestyle='--', color='C1')
        axs[0].set_ylabel('Error')
        axs[0].legend()
        

        axs[1].plot(time_axis, xyz_error, label='RMSE taskspace', color='C0')
        axs[1].plot(time_axis, nopti_xyz_error, label='No adapt. RMSE taskspace', linestyle='--', color='C0')
        mean_error = np.convolve(xyz_error, np.ones(N_mean)/N_mean, mode='valid')
        mean_error_no_adapt = np.convolve(nopti_xyz_error, np.ones(N_mean)/N_mean, mode='valid')
        axs[1].plot(time_axis[-len(mean_error):], mean_error, label='Loop average', color='C1')
        axs[1].plot(time_axis[-len(mean_error_no_adapt):], mean_error_no_adapt, label='No adapt. Loop average', linestyle='--', color='C1')
        ...
        for i in range(1, len(opti_index)):
            ...
            axs[0].axvline(x=time_loc, color='r', linestyle='--', alpha=0.5, linewidth=0.8)
            axs[1].axvline(x=time_loc, color='r', linestyle='--', alpha=0.5, linewidth=0.8)
...
def save_error(pcc_arm,u_bound,xyz_traj=None, save=False, opti_index=None, sim_parameters=None):
    history_pred = pcc_arm.history_pred[:, :pcc_arm.history_index].T
    history_meas = pcc_arm.history_meas[:, :pcc_arm.history_index].T
    ...
    history_pred_xyz = np.array([pcc_arm.end_effector(x[:2*pcc_arm.num_segments]).full().flatten() for x in history_pred[:-1,:]])
    xyz_error = np.linalg.norm(history_xyz_meas - history_pred_xyz, axis=1)
    np.save(out_dir + "q_error.npy", q_error)
    np.save(out_dir + "xyz_error.npy", xyz_error)
