...
def history_plot(pcc_arm,u_bound):
    ...
    for i in range(pcc_arm.num_segments):
        ...
        labels = [r'$\phi$', r'$\phi_d$', r'$\dot{\phi}$', r'$\dot{\phi}_d$']
        linestyle = ['-', '--', '-', '--']
        color = ['b', 'b', 'c', 'c']
        idx = np.array([2*i, 4*pcc_arm.num_segments+2*i, 2*pcc_arm.num_segments+2*i, 4*pcc_arm.num_segments+2*pcc_arm.num_segments+2*i]) 
        for j in range(4):
        ...
        labels = [r'$\theta$', r'$\theta_d$', r'$\dot{\theta}$', r'$\dot{\theta}_d$']
        linestyle = ['-', '--', '-', '--']
        color = ['b', 'b', 'c', 'c']
        idx = np.array([2*i+1, 4*pcc_arm.num_segments+2*i+1, 2*pcc_arm.num_segments+2*i+1, 4*pcc_arm.num_segments+2*pcc_arm.num_segments+2*i+1])
        for j in range(4):
    ...
    for i in range(pcc_arm.num_segments):
        for k in range(3):
            axs[i].plot(time, history_u.T[i+k,:], label=labels[k], linestyle='-', color=color[k])
        axs[i].set_title(f'Segment {i+1}')
        axs[i].set_xlabel('Time [s]'); axs[i].set_ylabel('N'); axs[i].legend()
        axs[i].set_ylim([0, u_bound])
...
def normalize(M,num_segments, eps=1e-8):
    ...
    divider = np.hstack([angle_divider, velocity_divider, angle_divider, velocity_divider])
